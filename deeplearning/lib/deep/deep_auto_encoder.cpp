...
//
// void deep_auto_encoder::make(const shared_ptr<deep_ae_param> & param_)
// {
// 	param =  param_;
//
...
// #pragma endregion
	...
	const MatrixType & deep_auto_encoder::get_last_layer_activation()
	{
		return lastLayerActivation;
	}
	...
	void deep_auto_encoder::zero_dWb()
	{
		...
		void deep_auto_encoder::add_performance_evaluator(const shared_ptr<evaluator_dep<deep_auto_encoder,NumericType>> & perf_evaluator_)
		{
		  perf_evaluators.push_back(perf_evaluator_);
		}
		...
		for (int level = coder_layer_id+1; level < num_layers; level ++)
		{
			...
			else
			{
				if (level == num_layers - 1)
					...
					logistic_layer_output(layered_input[level+1], lastLayerActivation,*W[level], 'N', layered_input[level], *b[level]);
...
//				this->save_hdf(boost::lexical_cast<string>(curr_iter) + ".hdf");
	...
	int deep_auto_encoder::get_finetune_running_iter()
	{
		return finetune_running_iter;
	}

	void deep_auto_encoder::progress_notified(int iterNum)
	{

		finetune_running_iter = iterNum;

		if (batch_size != 0 && mini_batch_opt_finished == false)
			return;

		if (perf_evaluators.size() == 0)
			return;

		vector<double> param;

		for(int i = 0; i<perf_evaluators.size();i++)
		  {
		    perf_evaluators[i]->evaluate(*this,param);
		  }
